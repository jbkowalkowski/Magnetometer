...
#define BEACON_TARGET "127.0.0.1"
#define BEACON_INTERVAL_MS 1000

using udp_ptr = std::unique_ptr<udp_pcb, decltype(&udp_remove)>;
using pcb_ptr = std::unique_ptr<pbuf, decltype(&pbuf_free)>;

void run_udp_beacon()
{
    auto pcb = udb_ptr(udp_new(), &udp_remove);
    ip_addr_t addr;
    ...
    int counter = 0;
    while (true) {
        auto pbuf p = pbuf_ptr(pbuf_alloc(PBUF_TRANSPORT, BEACON_MSG_LEN_MAX+1, PBUF_RAM), &pbuf_free);
